...
# 2) Imports
# ——————————————————————————————————————————————
from football_debate.llm_interface.ollama_interface import generate_response
...
def start_debate(
    ...
    logger.info(
        "DÉBUT DÉBAT — Format:%s — Tours:%d — OM Persona:%s — PSG Persona:%s",
        debate_format, max_turns, om_personality, psg_personality
    )
    ...
    raw_om = generate_response(prompt_om)
    ...
    raw_psg = generate_response(prompt_psg)
    ...
    while turn < max_turns:
        ...
        raw_om = generate_response(prompt_om_follow)
        ...
        raw_psg = generate_response(prompt_psg_follow)
        psg_resp = sanitize_response("PSG", raw_psg, previous=om_resp)
        transcript.append({"speaker": "PSG", "message": psg_resp})
        turn += 1
        time.sleep(1)
